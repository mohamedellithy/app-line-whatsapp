<?php
namespace App\AppServices\AppSettings;

use Log;
abstract class Events
{
    public $events = array();
    public $data   = array();
    public function events(){
        return [
          'app.store.authorize'      => 'SallaAuthorize',
          'abandoned.cart'           => 'SallaAbandonedCart',
          'app.settings.updated'     => 'SallaSettingsUpdate',
          'app.subscription.started' => 'SallaSubscription',
          'app.subscription.renewed' => 'SallaSubscription',
          'order.created'            => 'SallaOrder',
          'order.updated'            => 'SallaOrder',
          'customer.otp.request'     => 'SallaOtpRequest',
          'customer.created'         => 'SallaCustomerCreated',
          'manual.review.request'    => 'SallaManualReviewRequest'
        ];
    }

    public function get_json_data(){
        $request_data = file_get_contents('php://input');
        return json_decode($request_data);
    }

    public function make_event(){
        // identity events
        $this->events = $this->events();

        // identity data from request api
        $this->data  =  $this->get_json_data();

        if(!$this->events[$this->data->event]){
            echo 'Are trying to cheat us ? or just lost. </br> please Contact our support for help :) 905050555441 | support@karzoun.app';
            $log = json_encode($this->data, JSON_UNESCAPED_UNICODE) . PHP_EOL;
            Log::channel('not_listened_events')->info($log);
            return;
        }

        // call events and render it
        $target_event  =  new $this->events[$this->data->event]($this->data);
        $target_event->resolve_event();
    }
}
